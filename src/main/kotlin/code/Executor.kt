...
interface Invokable { //TODO: a debug invoker for debugging, and maybe misc invokers for fun
    val name: String
...
class ExecController(ctx: InvokeContext, private val code: StoreWorldCode, startIndex: Int) {
    private var lastFrame: ExecFrame? = null
    var frame = ExecFrame(ctx.label, ctx.instList, startIndex)

    operator fun hasNext() = frame.instIndex <= frame.instList.size-1
