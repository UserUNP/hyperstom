...
@Serializable(EventTargetSerializer::class) class EventTarget<T : InstanceEvent>(
    name: String,
    eventType: KClass<T>,
    get: EventDataContext<T>.() -> Set<Entity>,
) : EventDataProcessor<T, Set<Entity>>(name, eventType, get)
...
val TARGET_NPC_ALL = reg("NPC_ALL", InstanceEvent::class) { event.instance.entities.filter { it !is Player }.toSet() }
val TARGET_NPC_RAND = reg("NPC_RAND", InstanceEvent::class) { setOf(TARGET_NPC_ALL.get(this).random()) }
val TARGET_DEFAULT = reg("DEFAULT", EntityInstanceEvent::class) { setOf(event.entity) }
// event specific
val TARGET_ENTITY_CLICKED = reg("ENTITY_CLICKED", PlayerEntityInteractEvent::class) { setOf(event.entity) }

private fun <T : InstanceEvent> reg(n: String, e: KClass<T>, g: EventDataContext<T>.() -> Set<Entity>)
