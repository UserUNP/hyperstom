package hyperstom.infernity.dev.code.event

import hyperstom.infernity.dev.code.interpreter.InterpreterThread
import net.kyori.adventure.text.Component

private object DefaultEvent : CodeEvent {
    override val isDefault = true
    override val description = listOf<Component>()

    override fun interpreterThread(): InterpreterThread {
        TODO("Not yet implemented")
    }
}

private enum class PlayerCodeEvents : CodeEvent {
    Join {
        override val isDefault = false
        override val description: List<Component> = listOf()
        override fun interpreterThread() = InterpreterThread()
    },
}

object PlayerEvents : CodeEvent.Registry() {
    override val default: CodeEvent = DefaultEvent
    override fun get(index: Int): CodeEvent = PlayerCodeEvents.entries[index]
    override fun get(name: String): CodeEvent {
        if (name.isEmpty()) return default
        return PlayerCodeEvents.valueOf(name)
    }
}
